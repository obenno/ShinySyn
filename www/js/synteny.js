...
var search_micro_queryBed = null;
Shiny.addCustomMessageHandler("searchMicroSynteny_queryBed", function(inputShinyData){
    search_micro_queryBed = convertShinyData(inputShinyData);
    //console.log(search_micro_queryBed);
});

var search_micro_subjectBed = null;
Shiny.addCustomMessageHandler("searchMicroSynteny_subjectBed", function(inputShinyData){
    search_micro_subjectBed = convertShinyData(inputShinyData);
    //console.log(search_micro_subjectBed);
});

var search_micro_anchors = null;
Shiny.addCustomMessageHandler("searchMicroSynteny_anchors", function(inputShinyData){
    search_micro_anchors = convertShinyData(inputShinyData);
    //console.log(search_micro_anchors);
});

var search_syntenyError = 0;

Shiny.addCustomMessageHandler("plot_searchMicroSynteny", plot_searchMicroSynteny);

function plot_searchMicroSynteny(x){

    // prepare input data first
    let pathData = search_micro_anchors.map((d) => {
        return {
            from: d.q_Gene,
            to: d.s_Gene,
            sourceElementID: d.q_Gene.replace(/\./g, "_"),
            targetElementID: d.s_Gene.replace(/\./g, "_"),
            sourceStrand: d.q_GeneStrand,
            targetStrand: d.s_GeneStrand
        };
    });

    pathData = pathData.filter(d => d.to != ".");
    console.log(pathData);

    // Extract query data and plot
    let queryData = search_micro_queryBed.map(d => {
        return {
            gene: d.gene,
            chr: d.chr,
            start: d.start,
    ...
    // Extract subject data and plot
    let subjectDataStart = d3.min(search_micro_subjectBed.map(d => d.start));
    let sbujectDataEnd = d3.max(search_micro_subjectBed.map(d => d.end));
    // Fake selectedRegionData
    let selectedRegionData = [
        {
            anchors: [{s_GeneStart: subjectDataStart,
                       s_GeneEnd: sbujectDataEnd}],
            queryGenes: queryData,
            pathData: pathData
    }];

    d3.select("#geneSearch_microSynteny")
        .select("svg").remove(); // remove svg first
    const svg = generate_microSytenySVG(selectedRegionData, search_micro_subjectBed,
                                        microSyntenyHeight,
                                        microSyntenyMargin,
                                        "searchSyntenyQueryGroup",
                                        "searchSyntenySubjectGroup",
                                        "searchSyntenyRibbons");
    d3.select("#geneSearch_microSynteny")
        .append(() => svg.node());
...
Shiny.addCustomMessageHandler("highlightSelectedGene", function(inputShinyData){
    let selectedTransID = inputShinyData;
    d3.select("#" + selectedTransID.replace(/\./g, "_"))
        .style("fill", "#FF5733");
});

Shiny.addCustomMessageHandler("remove_searchSyntenySVG", function(inputShinyData){
    d3.select("#geneSearch_microSynteny")
        .select("svg").remove();
});
